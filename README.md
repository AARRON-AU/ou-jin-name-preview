...
除 40 个金、水候选字外，页面还提供 21 个兄弟呼应字。手动输入未收录字时会立即按内置的金水字形规则或中性保守规则给出分数和逐项依据，同时提示粤语读音需要人工复核。普通话按钮使用标准普通话在线音频，粤语按钮使用设备的 `zh-HK/yue` 语音；两者都用“欧晋X”朗读，以保证“区”的普通话和粤语姓氏读音都按“欧”处理。普通话音频需要联网。
